...
- [ ] **E6-S1** — Back: `NoteVersion` model & migration (composite index)
- [ ] **E6-S2** — Back: create a version on session end + `GET .../versions`
- [ ] **E6-S3** — Back: `POST .../restore/{version_id}` (creates a new version)
- [ ] **E6-S4** — Front: HistoryPanel desktop (list + preview + restore)
- [ ] **E6-S5** — Front: mobile 2-screen flow (list → preview → Fermer / Restaurer)
- [ ] **E6-S6** — Tests: one-per-session, restore = new version, visibility gating
...
**Acceptance criteria**
- [ ] `NoteVersion` table + `(note_id, created_at)` composite index created by a
  checked-in migration.
- [ ] A version stores a full snapshot (title, body, color, visibility, author, time).
- [ ] Postgres-safe.
...
**Acceptance criteria**
- [ ] One editing session produces **at most one** version (not per keystroke, FR-H1).
- [ ] Public-note version is written on lock release; private-note version on close.
- [ ] `GET .../versions` returns who + when, newest-first, gated by visibility.
- [ ] A session with no change does not create a version.
...
**Acceptance criteria**
- [ ] Restore makes the note's content equal the chosen version.
- [ ] A **new** version is appended; **nothing is overwritten** (FR-H4).
- [ ] The previously-current version remains in history untouched.
...
**Acceptance criteria**
- [ ] List shows who/when, newest-first, with the "actuelle" badge.
- [ ] Selecting a version previews it read-only (re-rendered as-is, no diff).
- [ ] Restore asks for confirmation with the exact copy, then creates a new version.
- [ ] Faithful in light + dark.
...
**Acceptance criteria**
- [ ] Mobile flow: tap a row → preview → Fermer / Restaurer bar (frozen decision).
- [ ] Preview is read-only with the gold "Aperçu" banner + author/date.
- [ ] Restore from mobile creates a new version.
...
**Acceptance criteria**
- [ ] One-version-per-session tested (FR-H1).
- [ ] Restore-creates-new-version, nothing overwritten, tested (FR-H4).
- [ ] Private-history gating tested (FR-H2).
- [ ] CI green.
...
- [ ] History lists who + when, newest-first, visibility-gated.
- [ ] Read-only preview (desktop panel + mobile 2-screen), re-rendered as-is (no diff).
- [ ] One version per editing session (public: lock release; private: editor close).
- [ ] Restore creates a new version; nothing is ever overwritten.
- [ ] Versioning tests green in CI.
