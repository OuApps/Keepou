...
- Board controls: search reset button label « Effacer la recherche » · sort selector (label « Trier les notes ») « Date de modification » / « Date de création » / « Titre (A→Z) ». (Separating own private vs. public notes is handled by the top-right « Mes notes » / « Public » tab, so there is no separate visibility filter.)
