...
- [x] **E11** — Field-feedback follow-up · ✅ detailed → [`stories/E11-retours-terrain.md`](./stories/E11-retours-terrain.md) — *shipped (board sort + density selectors + search reset + year-in-old-dates + return-state; hard delete from the card, the editor and archive multi-select/select-all; owner pin/archive/delete + `Maj+Entrée` in the editor; self-service display-name change `PATCH /api/auth/me`; windowed board rendering; instant open/close via a board cache with optimistic upserts; editor « Supprimer les cases cochées » to clear all ticked boxes (S6); editor « Copier la note » one-click whole-note copy (S7); **round-2 edit-mode fixes** — drag-select no longer closes the editor, blank lines survive a session, mid-note checkbox insertion (`[]` shorthand + insert-at-caret), Backspace/Suppr merges at block edges, stable clear-checked button, « Aperçu » density by default, tighter mobile spacing; **round-3 edit-mode fixes** — ↑/↓/←/→ walk the caret across blocks, blank lines preserved verbatim incl. between two boxes (HANDOFF §3.3 relaxed), Enter mid-label splits the item; tests back & front)*
