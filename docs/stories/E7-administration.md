...
> All stories are **done** — the epic shipped.
...
**Status.** All **done**. Built on `require_admin` from E2-S2: `routers/admin.py`
(4 endpoints), `AccessManager` + `MemberRow` / `PendingRow` at `/admin`, the
admins-only « Administration » entry in the avatar menu, `styles/admin.css`,
17 pytest + 9 Vitest tests.
